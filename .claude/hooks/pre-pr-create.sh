...
#   - the org-wide work-in-progress cap (see WIP_CAP below; ready-for-review
#     pull requests per
...
# Org-wide work-in-progress cap: ready-for-review pull requests per author,
# across every Proof-labs repository. Raised 5 → 15 on 2026-07-26 (Ramon):
# 5 throttled authors who legitimately have several small changes in flight,
# which pushed real work into drafts and made the draft state meaningless.
# Change it HERE — the messages below interpolate it.
WIP_CAP = 15
...
def _is_decided(cell):
    ...
    Strip markdown emphasis and surrounding punctuation first, then compare
    case-insensitively. Statuses are Open / Blocked / Decided, so a prefix
    match is unambiguous once the emphasis is gone.
    """
...
def validate(seg):
    ...
    # ── org-wide work-in-progress cap: WIP_CAP ready-for-review PRs per author ──
    ...
    if "cap" not in skip:
        ...
        if count >= WIP_CAP:
            block(f"PR creation blocked: work-in-progress cap reached — you "
                  ...
                  f"Proof-labs (cap: {WIP_CAP}). Open this one with --draft, or get an "
