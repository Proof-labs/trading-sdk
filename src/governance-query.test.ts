...
/** `AdminAction::CreateEvent` (inner tag 9) — the `action` slot of the
 *  `propose_admin_action/create_event` conformance vector, generated by the
 *  Rust core (crates/spec gen-vectors) for the E1 golden event: 700, books
 *  from 70000, "Will the Fed cut rates?", settlement 1778000000000, nil
 *  oracle source and empty text trailers. */
const ACTION_CREATE_EVENT =
  "81ab4372656174654576656e749ccd02bcce0001117000b757696c6c2074686520466564206375742072617465733fcf0000019df90ef400cd03e80502dc00140000000000000000000000000000000000000000c0a0a0";
...
describe("decodeAdminAction", () => {
  ...
  it("decodes the CreateEvent bytes of the create_event conformance vector (admin tag 9)", () => {
    expect(decodeAdminAction(decodeVector(ACTION_CREATE_EVENT))).toEqual({
      kind: "CreateEvent",
      value: {
        eventId: 700,
  ...
  it("tolerates the CreateEvent serde(default) trailers' absence, like the engine", () => {
    // 9 slots — the required fields only, no oracle-source/description/rules
    // trailers. The engine's decoder defaults them; the mirror must too.
    const bare = {
      CreateEvent: [
        700,
        70_000,
    ...
    const action = decodeAdminAction(bare);
    if (action.kind !== "CreateEvent") throw new Error("expected CreateEvent");
    expect(action.value.eventId).toBe(700);
    expect(action.value.settlementMs).toBe(1_778_000_000_000n);
    expect(action.value.oracleSource).toBeUndefined();
    expect(action.value.description).toBe("");
    expect(action.value.rules).toBe("");
  });
